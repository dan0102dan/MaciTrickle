<script lang="ts">
  ...
  import { isDirectList, type Rule } from "../../../types";
    ...
    <Collapsible.Root open={effectiveOpen} onOpenChange={toggleOpen}>
      ...
      <Collapsible.Content>
        <div transition:slide={searchActive ? { duration: 0 } : {}}>
          {#if isDirectList(group)}
            <p class="group-direct-note">
              {t(
                "This group routes nothing: the addresses it collects are left alone, and every other group skips them.",
              )}
            </p>
          {/if}
