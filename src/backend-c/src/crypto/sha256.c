...
static void sha256_block(mt_sha256_ctx_t *ctx, const uint8_t block[64]) {
    uint32_t w[64];
...
void mt_sha256_final(mt_sha256_ctx_t *ctx, uint8_t out[MT_SHA256_DIGEST_LEN]) {
    ...
    uint8_t pad = 0x80;
    mt_sha256_update(ctx, &pad, 1);
    uint8_t zero = 0;
    while (ctx->buf_len != 56) {
        mt_sha256_update(ctx, &zero, 1);
    }
    uint8_t len_bytes[8];
    for (int i = 0; i < 8; i++) {
        len_bytes[7 - i] = (uint8_t)(bit_len >> (8 * i));
    }
