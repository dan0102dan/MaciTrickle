...
static void sha512_block(mt_sha512_ctx_t *ctx, const uint8_t block[128]) {
    uint64_t w[80];
...
void mt_sha512_final(mt_sha512_ctx_t *ctx, uint8_t out[MT_SHA512_DIGEST_LEN]) {
    ...
    uint8_t pad = 0x80;
    mt_sha512_update(ctx, &pad, 1);
    uint8_t zero = 0;
    while (ctx->buf_len != 112) {
        mt_sha512_update(ctx, &zero, 1);
    }
    /* 128-bit big-endian length field; high 64 bits are always zero for
     * any input we will ever see (see hash.h). */
    uint8_t len_bytes[16] = {0};
    for (int i = 0; i < 8; i++) {
        len_bytes[15 - i] = (uint8_t)(bit_len >> (8 * i));
    }
    memcpy(ctx->buf + 112, len_bytes, 16);
    sha512_block(ctx, ctx->buf);

    for (size_t i = 0; i < 8; i++) {
        for (size_t j = 0; j < 8; j++) {
            out[i * 8 + j] = (uint8_t)(ctx->state[i] >> (8 * (7 - j)));
        }
    }
}
...
void mt_sha512(const uint8_t *data, size_t len, uint8_t out[MT_SHA512_DIGEST_LEN]) {
    mt_sha512_ctx_t ctx;
    mt_sha512_init(&ctx);
    mt_sha512_update(&ctx, data, len);
    mt_sha512_final(&ctx, out);
}
