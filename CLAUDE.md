...
`direct` is a reserved interface name alongside `blackhole`, offered for both
groups and subscriptions. A group or subscription pointed at it routes
nothing: it builds no chain, mark, ip rule or route, and its ipset exists
only to be skipped. Every routing group's mangle chain leaves on every
enabled direct set before marking:

```
-m conntrack --ctdir REPLY -j RETURN
-m set --match-set mt_<direct-list>_4 dst -j RETURN   # one per direct list
-m set --match-set mt_<self>_4 dst -j MARK --set-mark <mark>
```

So the usual setup is two groups: one `direct` list naming what to ignore,
one group with a catch-all pattern pointed at the tunnel. The leave-rules
are what makes this work — the DNS path adds a resolved address to *every*
group whose rules match, so a wildcard group would otherwise route the very
domains the direct list names. `refresh_bypass_sets` in `src/api/app.c`
keeps the two sides linked (after every mutation, on every rebuild, and once
at startup). See `docs/c-rewrite/decisions.md` D-57.
